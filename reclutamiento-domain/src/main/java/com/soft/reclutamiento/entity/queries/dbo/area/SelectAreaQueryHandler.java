...
/**
 * Handler para SelectAreaQuery
 * Extiende de SearchQueryHandlerBase para validación automática de búsqueda
 */
@Component
public class SelectAreaQueryHandler extends SearchQueryHandlerBase<SelectAreaQuery, SelectAreaFilterDto, SelectAreaDto> {

    private final AreaRepository areaRepository;
    private final AreaMapper areaMapper;

    public SelectAreaQueryHandler(Messages messages, AreaRepository areaRepository, AreaMapper areaMapper) {
        super(messages);
    ...
    @Override
    protected ResponseDto<SearchResultDto<SelectAreaDto>> handleQuery(SelectAreaQuery query) {
        var response = new ResponseDto<SearchResultDto<SelectAreaDto>>();
        ...
        Specification<Area> specification = (root, criteriaQuery, criteriaBuilder) -> {
            ...
            // Filtros adicionales desde SelectAreaFilterDto
        ...
        // Mapear a DTOs
        List<SelectAreaDto> areaDtos = searchResult.getItems().stream()
                .map(areaMapper::toSelectAreaDto)
                .collect(Collectors.toList());

        SearchResultDto<SelectAreaDto> resultDto = new SearchResultDto<>(
                areaDtos,
