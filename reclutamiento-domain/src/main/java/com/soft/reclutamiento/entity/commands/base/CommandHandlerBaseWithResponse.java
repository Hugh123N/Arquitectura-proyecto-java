...
/**
 * Clase base para handlers de Commands con respuesta específica
 * Maneja automáticamente la validación antes de ejecutar el comando
 */
public abstract class CommandHandlerBaseWithResponse<TCommand extends CommandWithResponse<TResponse>, TResponse>
        implements Command.Handler<TCommand, ResponseDto<TResponse>> {

    protected final Messages messages;
    protected final CommandValidatorBase<TCommand> validator;

    protected CommandHandlerBaseWithResponse(Messages messages, CommandValidatorBase<TCommand> validator) {
    ...
    @Override
    @Transactional
    public ResponseDto<TResponse> handle(TCommand command) {
     ...
     */
    protected abstract ResponseDto<TResponse> handleCommand(TCommand command);

    /**
     * Agrega información de excepción al response
     */
    protected void addExceptionCommandResult(ResponseDto<TResponse> response, Exception exception) {
